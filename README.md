...
- `答 1:A 2:BD 提交`：填入答案后立即尝试提交；仍有未答题时不会提交。
- `提交`、`确认提交`、`交卷`、`提交答案`：在所有题目已作答后提交。
- `帮助`、`菜单`、`help`、`?`：查看完整命令菜单。
- `状态`：查看当前视频播放进度、答题进度和是否可提交。
- `答题进度`：只查看题目已答/未答状态。
- `播放进度`、`视频进度`、`当前进度`：查看当前视频播放时间、总时长、百分比和播放状态。
- `当前`、`题目`、`题目 3`、`题目 1 2`：查看当前未答题或指定题目的文字内容。
- `重发` 或 `题目全部`：重发本页全部文字题目。
- `题图 1`、`截图 1`、`图片 1` 或 `题图全部`：重发题目截图。
- `解析 1`、`讲解 1`、`解释 1` 或 `解析全部`：请求学习型解析。
- `错题` 或 `错题 10`：查看最近错题记录。
- `清空错题`：清空本地错题库。
